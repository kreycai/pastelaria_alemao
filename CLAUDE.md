...
| `/cozinha`                   | Client | Display da cozinha (3 colunas, polling de 5s)      |
...
### WebSocket (socket.io) — ESBOÇADO, NÃO LIGADO
Existe um gateway em `apps/api/src/modules/pedido/orders.gateway.ts` com os métodos
`emitNovoPedido` e `emitStatusAtualizado`, mas **nada o injeta e nada assina os eventos** —
é código morto por enquanto. As telas de cozinha (web e mobile) atualizam por polling.

Para ligar de verdade: injetar `OrdersGateway` no `PedidoService`, chamar os dois métodos em
`create()` e `updateStatus()`, e assinar `novo-pedido` / `status-atualizado` com
`socket.io-client` em `/cozinha` e na aba de cozinha do mobile.
